...
A local web application for learning a foreign language (initially Slovenian) aimed at an American English speaker. The app teaches vocabulary and grammar *in context* by presenting glossed sentences and lemma (word) pages. Sentence content is generated eagerly on RSS import, while lemma content is generated on demand; all generated content is cached in a local SQLite database.
...
- Generate lemma content on demand to minimize LLM costs; cache all generated content.
...
- Single-user application designed for personal use on personal devices.
...
- **Access counting**: Each successful full page load that renders content (HTTP 200) increments the sentence's access count by 1. HTMX partial updates, regenerations, and redirects do not increment counts.
...
- Show favorites link.
- Include a "Show glosses" toggle.
  - Default: on.
  - Storage: per-session (Flask session).
  - Behavior: toggling updates the page in place (HTMX, no history push) and applies via CSS (e.g., a class on the root element) so newly swapped-in content respects the current setting.
...
- Support fast lookups of sentences containing a lemma.
...
All timestamps are stored as ISO 8601 UTC strings in `TEXT` columns.
...
- When a sentence is deleted, also delete its `sentence_lemmas` rows (DELETE FROM sentence_lemmas WHERE sentence_id=?).
...
- Articles are deduplicated globally by the RSS item's `id`/GUID field as returned by the RSS parser.
  ...
  - Store each deduplication id in `rss_articles` as a single global list (no feed_id).
  - Global dedup may drop rare cross-feed duplicates (including across languages); this is acceptable.
